...
contract Presale is Ownable, ReentrancyGuard {
    ...
    function finalize(
        ...
        //handle create pair if pair not existed
        if (createPairAddress.getPair(WBNB, address(token)) == address(0)) {
    ...
    function getLiquidAmount() public view returns (uint256) {
        return ((totalSold * rateListing * liquidListingPercentage) / 100);
    }
}
